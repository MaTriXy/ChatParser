...
describe("Parses Smashcast", () => {
	...
	describe('works without emotes', () => {
		beforeEach(() => {
			parser = new Parser();
			parsed = parser.parseMessage(ChatPlatform.Smashcast, Example.smashcastJSON);
		});
		...
		it('Thinks the user is a subscriber', () => {
			...
			it("Should have the correct user object", () => {
				...
				expect(parsed.user.userId).to.equal(null);
			});
		})
		...
		describe('parses message correctly', () => {
			...
			it('doesn\'t see the emoticon', () => {
				expect(parsed.message[1].type).to.equal('text');
			})
	...
	describe('works with emotes', () => {
		...
		beforeEach(() => {
			...
			parsed = parserEm.parseMessage(ChatPlatform.Smashcast, Example.smashcastJSON);
		});
